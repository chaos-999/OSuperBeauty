...
int virtio_pci_read_caps(virtio_pci_hw_t *hw, uint64 pci_base, trap_handler_fn *msix_isr) {
    ...
    // printf("cap: 0x%016llx\n", pci_base + PCI_ADDR_CAP);     // cap pointer 在 ECAM 整个区域中的
    // offset
    ...
    if (hw->common_cfg == NULL || hw->notify_cfg == NULL || hw->device_cfg == NULL ||
        hw->isr_cfg == NULL) {
...
void virtio_pci_set_queue_addr2(virtio_pci_hw_t *hw, int qid, void *desc, void *avail, void *used) {
    ...
    PCI_REG16(&cfg->queue_select) = qid;
    dsb();
    // printf("%x %x %x\n", ((uint64)desc) & (~(DMWIN_MASK)), ((uint64)avail) & (~(DMWIN_MASK)),
    // ((uint64)used) & (~(DMWIN_MASK))); PCI_REG64(&cfg->queue_desc) = ((uint64)desc) &
    // (~(DMWIN_MASK)); PCI_REG64(&cfg->queue_driver) = ((uint64)avail) & (~(DMWIN_MASK));
...
int virtio_pci_setup_queue(virtio_pci_hw_t *hw, struct vring *vr) {
    ...
    vr->notify_addr = (void *)((uint8 *)hw->notify_cfg + notify_off * hw->notify_off_multiplier);
