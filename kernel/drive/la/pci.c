...
void pci_read_config(unsigned long base_cfg_addr, unsigned int bus, unsigned int device,
    ...
    unsigned long pcie_header_base =
        DMWIN1_MASK | base_cfg_addr | (bus << 16) | (device << 11) | (function << 8);

    *read_data = *(volatile unsigned int *)(pcie_header_base + reg_id);
}

void pci_write_config(unsigned long base_cfg_addr, unsigned int bus, unsigned int device,
                      unsigned int function, unsigned int reg_id, unsigned int write_data) {
    unsigned long pcie_header_base =
        DMWIN1_MASK | base_cfg_addr | (bus << 16) | (device << 11) | (function << 8);
...
/*初始化pci设备信息*/
static void pci_device_init(pci_device_t *device, unsigned char bus, unsigned char dev,
                            unsigned char function, unsigned short vendor_id,
                            unsigned short device_id, unsigned int class_code,
                            unsigned char revision_id, unsigned char multi_function) {
    /*设置驱动设备的信息*/
    device->bus = bus;
    device->dev = dev;
...
static void pci_device_bar_init(pci_device_bar_t *bar, unsigned int addr_reg_val,
                                unsigned int len_reg_val) {
...
void pci_scan_device(unsigned char bus, unsigned char device, unsigned char function) {
    ...
    pci_read_config(PCI_CONFIG0_BASE, bus, device, function,
                    PCI_BIST_HEADER_TYPE_LATENCY_TIMER_CACHE_LINE, &val);
    ...
    /*初始化pci设备*/
    pci_device_init(pci_dev, bus, device, function, vendor_id, device_id, classcode, revision_id,
                    (header_type & 0x80));
    ...
    /*获取中断相关的信息*/
    pci_read_config(PCI_CONFIG0_BASE, bus, device, function, PCI_MAX_LNT_MIN_GNT_IRQ_PIN_IRQ_LINE,
                    &val);
...
pci_device_t *pci_get_device(unsigned int vendor_id, unsigned int device_id) {
    ...
    for (i = 0; i < PCI_MAX_DEVICE_NR; i++) {
        ...
        if (device->flags == PCI_DEVICE_USING && device->vendor_id == vendor_id &&
            device->device_id == device_id) {
            return device;
        }
    }
...
pci_device_t *pci_get_device_by_bus(unsigned int bus, unsigned int dev, unsigned int function) {
    ...
    for (int i = 0; i < PCI_MAX_DEVICE_NR; i++) {
        ...
        if (tmp->bus == bus && tmp->dev == dev && tmp->function == function) {
            return tmp;
        }
    }
    return NULL;
...
uint8 pci_config_read8(uint64 offset) { return PCI_REG8(offset); }

uint16 pci_config_read16(uint64 offset) { return PCI_REG16(offset); }

uint32 pci_config_read32(uint64 offset) { return PCI_REG32(offset); }

uint64 pci_config_read64(uint64 offset) { return PCI_REG64(offset); }

void pci_config_write8(uint32 offset, uint8 val) { PCI_REG8(offset) = val; }

void pci_config_write16(uint32 offset, uint16 val) { PCI_REG16(offset) = val; }

void pci_config_write32(uint32 offset, uint32 val) { PCI_REG32(offset) = val; }
